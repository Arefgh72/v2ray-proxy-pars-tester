...
def download_and_extract_xray():
    """
    آخرین نسخه Xray را با استفاده از curl (برای پایداری بیشتر) دانلود و استخراج می‌کند.
    """
    ...
    try:
        # دریافت اطلاعات آخرین نسخه با requests
        ...
        if not download_url:
            log_error("Xray Download", "Could not find download URL for Xray-linux-64.zip.")
            return False
            
        print(f"Downloading from: {download_url}")
        
        # --- تغییر اصلی: استفاده از curl برای دانلود فایل ---
        download_command = ["curl", "-L", "-o", "xray.zip", download_url]
        subprocess.run(download_command, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        ...
        error_details = e.stderr.decode('utf-8') if isinstance(e, subprocess.CalledProcessError) else str(e)
        log_error("Xray Download", "An error occurred during download or extraction.", error_details)
        return False
...
def parse_proxy_link_to_xray_outbound(proxy_url: str):
    try:
        ...
        log_error("Smart Parser", f"Failed for proxy: {proxy_url[:60]}...", str(e)); return None
def test_proxy(proxy_url: str) -> tuple[int, str]:
    thread_id = threading.get_ident(); config_filename = f"temp_config_{thread_id}.json"
    outbound_config = parse_proxy_link_to_xray_outbound(proxy_url)
    if not outbound_config: return -1, "CONFIG_FAILED"
    ...
    process = None
    try:
        process = subprocess.Popen([XRAY_EXECUTABLE_PATH, "-config", config_filename], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL); time.sleep(1.5)
        start_time = time.time(); proxies = {"http": f"http://127.0.0.1:{XRAY_LOCAL_PORT}", "https": f"http://127.0.0.1:{XRAY_LOCAL_PORT}"}
        response = requests.get(TEST_URL, proxies=proxies, timeout=TIMEOUT_SECONDS); end_time = time.time()
        if response.status_code == 200: return int((end_time - start_time) * 1000), "SUCCESS"
        else: return -1, f"HTTP_{response.status_code}"
    except requests.exceptions.Timeout: return -1, "TIMEOUT"
    except requests.exceptions.RequestException: return -1, "CONN_ERROR"
    finally:
...
def worker(proxy_queue, results_list, progress_lock):
    ...
    while not proxy_queue.empty():
        ...
        finally: proxy_queue.task_done()
def main():
    global total_proxies_to_test
    print("\n--- Running 02_test_github.py (Hiddify-style Smart Parser) ---")
    if not download_and_extract_xray(): sys.exit(1)
    try:
        with open(RAW_PROXIES_FILE, 'r') as f: proxies = [line.strip() for line in f if line.strip()]
        total_proxies_to_test = len(proxies); print(f"Found {total_proxies_to_test} proxies to test.")
    except FileNotFoundError: log_error("GitHub Test", f"'{RAW_PROXIES_FILE}' not found."); return
    if total_proxies_to_test == 0: print("No proxies to test."); return
    from queue import Queue
    proxy_queue = Queue();
    for p in proxies: proxy_queue.put(p)
    results = []; threads = []; num_threads = 50; progress_lock = threading.Lock()
    print(f"Starting test with {num_threads} threads...")
    for _ in range(num_threads):
        t = threading.Thread(target=worker, args=(proxy_queue, results, progress_lock)); t.daemon = True; t.start(); threads.append(t)
    proxy_queue.join()
    if total_proxies_to_test > 0: print(f"\nFinal Progress: 100.00% tested.")
    print(f"\nTest finished. Found {len(results)} working proxies.")
    if not results: print("No working proxies found.")
    sorted_results = sorted(results, key=lambda x: x['latency']); sorted_proxies = [item['proxy'] for item in sorted_results]
