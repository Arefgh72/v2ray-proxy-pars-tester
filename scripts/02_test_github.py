...
def download_and_extract_xray():
    if os.path.exists(XRAY_EXECUTABLE_PATH): return True
    ...
    try:
        ...
        if not download_url:
            log_error("Xray Download", "Could not find Xray-linux-64.zip.")
            return False
        ...
        with zipfile.ZipFile("xray.zip", 'r') as zip_ref: zip_ref.extract('xray')
        os.chmod(XRAY_EXECUTABLE_PATH, 0o755)
        os.remove("xray.zip")
        print("Xray downloaded successfully.")
        return True
    except Exception as e:
        log_error("Xray Download", "Failed to download/extract Xray.", str(e))
        return False
...
def parse_proxy_link_to_xray_outbound(proxy_url: str):
    ...
    این تابع از منطق کلاینت‌های پیشرفته مانند Hiddify الهام گرفته است.
    """
    try:
        ...
        # مقادیر پیش‌فرض را از پارامترهای URL استخراج می‌کنیم
        ...
        # --- ساخت StreamSettings (بخش کلیدی) ---
        stream_settings = {"network": network, "security": security}
        
        # مدیریت هوشمند TLS
        if security == 'tls':
            # SNI (Server Name Indication) اولویت بالایی دارد. اگر نباشد از Host استفاده کن. اگر آن هم نبود از آدرس سرور.
        ...
        # تنظیمات شبکه (WebSocket, gRPC)
        if network == 'ws':
            # Host هدر WebSocket اولویت دارد. اگر نباشد از SNI (اگر TLS فعال است) یا آدرس سرور استفاده کن.
        ...
        if protocol == "vless":
            ...
            if '@' in parsed_url.netloc: # حالت Base64
