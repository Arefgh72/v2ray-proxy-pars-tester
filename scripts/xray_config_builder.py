...
def build_xray_config(proxy_url: str, local_port: int):
    ...
    این نسخه شامل اصلاح Fingerprint برای TLS است.
    """
    try:
        parsed_url = urlparse(proxy_url)
        # ... (بقیه کد پارس کردن مثل قبل)
        ...
        if security == 'tls':
            ...
            fp = params.get('fp', ['random'])[0] # خواندن پارامتر fp از لینک

            # --- تغییر اصلی اینجاست ---
            tls_settings = {
                "serverName": sni,
                "fingerprint": fp # اضافه کردن فیلد فینگرپرینت
            }
            if alpn:
                tls_settings["alpn"] = alpn.split(',')
            stream_settings['tlsSettings'] = tls_settings

        # ... (بقیه کد ساخت کانفیگ مثل قبل)
        if network == 'ws':
        ...
        return {"log": {"loglevel": "warning"}, "inbounds": [{"port": local_port, "protocol": "socks", "listen": "127.0.0.1", "settings": {"auth": "noauth", "udp": True}}], "outbounds": [outbound]}
    except Exception:
        return None
